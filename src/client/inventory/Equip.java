...
public class Equip extends Item implements Serializable {
    ...
    public void resetStat(Equip ret) {
        str = ret.str;
        dex = ret.dex;
        _int = ret._int;
        luk = ret.luk;
        ...
        jump = ret.jump;
        enhance = ret.enhance;
        upgradeSlots = ret.upgradeSlots;
        level = ret.level;
        itemEXP = ret.itemEXP;
        durability = ret.durability;
        vicioushammer = ret.vicioushammer;
        potential1 = ret.potential1;
        potential2 = ret.potential2;
        potential3 = ret.potential3;
        potential4 = ret.potential4;
        potential5 = ret.potential5;
        bonuspotential1 = ret.bonuspotential1;
        bonuspotential2 = ret.bonuspotential2;
        bonuspotential3 = ret.bonuspotential3;
        fusionAnvil = ret.fusionAnvil;
        socket1 = ret.socket1;
        socket2 = ret.socket2;
        socket3 = ret.socket3;
        charmExp = ret.charmExp;
        pvpDamage = ret.pvpDamage;
        incSkill = ret.incSkill;
        enhanctBuff = ret.enhanctBuff;
        reqLevel = ret.reqLevel;
        yggdrasilWisdom = ret.yggdrasilWisdom;
        finalStrike = ret.finalStrike;
        bossDamage = ret.bossDamage;
        ignorePDR = ret.ignorePDR;
        totalDamage = ret.totalDamage;
        allStat = ret.allStat;
        karmaCount = ret.karmaCount;
        setGiftFrom(ret.getGiftFrom());
        setOwner(ret.getOwner());
        setQuantity(ret.getQuantity());
        setExpiration(ret.getExpiration());
        stats = ret.stats;
        specialStats = ret.specialStats;
        statsTest = ret.statsTest;
    }
    ...
    private byte modSlot = 0, upgradeSlots = 0, level = 0, vicioushammer = 0, enhance = 0, enhanctBuff = 0, reqLevel = 0, yggdrasilWisdom = 0, bossDamage = 0, ignorePDR = 0, totalDamage = 0, allStat = 0, karmaCount = -1;
    ...
    private int durability = -1, incSkill = -1, potential1 = 0, potential2 = 0, potential3 = 0, potential4 = 0, potential5 = 0, potential6 = 0, potential7 = 0, potential8 = 0, bonuspotential1 = 0, bonuspotential2 = 0, bonuspotential3 = 0, fusionAnvil = 0, socket1 = -1, socket2 = -1, socket3 = -1;
    ...
    public Equip(int id, short position, int uniqueid, short flag) {
        ...
        this.modSlot = ii.getReqLevel(id) < 100 ? (byte)(ii.getReqLevel(id) / 2) : (byte)(ii.getReqLevel(id) - 50);
    }
    ...
    public byte getModSlot() {
        return this.modSlot;
    }
    public void setModSlot(byte i) {
        modSlot = i;
    }
    ...
    public int blood() {
        ...
        LinkedList<List<StructItemOption>> pots = new LinkedList<List<StructItemOption>>(ii.getAllPotentialInfo().values());
            while (!rewarded) {
                ...
                if (pot == null || pot.reqLevel / 10 > reqLevel || !GameConstants.optionTypeFits(pot.optionType, getItemId()) || !GameConstants.potentialIDFits(pot.opID, new_state, 2) || !InventoryHandler.isAllowedPotentialStat(this, pot.opID)) continue;
    ...
    public void renewRed(boolean scroll, int type){
        int miracleRate = 1;
        if (EventConstants.DoubleMiracleTime) {
            miracleRate *= 2;
        }
        ...
        if(scroll)
            ...
            state = (Randomizer.nextInt(100) < 4 * miracleRate)? getState()+1:getState();
        if(state>20)
            state = 20;
        setPotential1(-state);
        if(scroll)
