...
//        }
        ...
        if (!GameConstants.isInno(scroll.getItemId()) && !GameConstants.isSpecialScroll(scroll.getItemId()) && !GameConstants.isCleanSlate(scroll.getItemId()) && !GameConstants.isEquipScroll(scroll.getItemId()) && !GameConstants.isPotentialScroll(scroll.getItemId())) {
        ...
        } else if (((GameConstants.isCleanSlate(scroll.getItemId()) || GameConstants.isInno(scroll.getItemId())) && scrolled.getUpgradeSlots() > oldSlots)) {
            scrollSuccess = Equip.ScrollResult.SUCCESS;
        } else {
            scrollSuccess = Equip.ScrollResult.FAIL;
        }
        ...
        scrolled.setUpgradeSlots((byte) (scrolled.getUpgradeSlots()+1));
    ...
    public static boolean UseCarvedSeal(final LittleEndianAccessor slea, final MapleClient c) {
        int new_state = 17;
        c.getPlayer().updateTick(slea.readInt());
        byte scroll = (byte)slea.readShort();
        byte dst = (byte)slea.readShort();
        Item toUse = c.getPlayer().getInventory(MapleInventoryType.USE).getItem(scroll);
        Equip toScroll = null;
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        LinkedList<List<StructItemOption>> pots = new LinkedList<List<StructItemOption>>(ii.getAllPotentialInfo().values());
        toScroll = dst < 0 ? (Equip)c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).getItem(dst) : (Equip)c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem(dst);
        short oldFlag = toScroll.getFlag();
        Equip.ScrollResult result = Equip.ScrollResult.FAIL;
        int reqLevel = ii.getReqLevel(toScroll.getItemId()) / 10;
        if (toScroll.getPotential3() > 0 || toScroll.getState() == 0) {
            c.getSession().write(CWvsContext.enableActions());
            c.getPlayer().dropMessage(1, "Sorry but this equip which have 3 potential lines or no potential.");
            return false;
        }
        boolean ok;
        int chance = (toUse.getItemId()==2049500)?50:20;
        ...
        if (toScroll.getPotential2() > 0) {
            ...
            while (!rewarded) {
                StructItemOption pot = pots.get(Randomizer.nextInt(pots.size())).get(reqLevel);
                if (pot == null || pot.reqLevel / 10 > reqLevel || !GameConstants.optionTypeFits(pot.optionType, toScroll.getItemId()) || !GameConstants.potentialIDFits(pot.opID, new_state, 2) || !InventoryHandler.isAllowedPotentialStat(toScroll, pot.opID)) continue;
                toScroll.setPotential3(pot.opID);
                rewarded = true;
            }
        } else if (toScroll.getPotential1() > 0) {
            boolean rewarded = false;
            while (!rewarded) {
                StructItemOption pot = pots.get(Randomizer.nextInt(pots.size())).get(reqLevel);
                if (pot == null || pot.reqLevel / 10 > reqLevel || !GameConstants.optionTypeFits(pot.optionType, toScroll.getItemId()) || !GameConstants.potentialIDFits(pot.opID, new_state, 1) || !InventoryHandler.isAllowedPotentialStat(toScroll, pot.opID)) continue;
                toScroll.setPotential2(pot.opID);
                rewarded = true;
            }
        }
        MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, scroll, (short) 1, false);
        c.getSession().write((Object)CWvsContext.InventoryPacket.scrolledItem(toUse, MapleInventoryType.EQUIP, toScroll, false, false, false));
        c.getPlayer().getMap().broadcastMessage(c.getPlayer(), CField.getScrollEffect(c.getPlayer().getId(), Equip.ScrollResult.SUCCESS, false, toScroll.getItemId(), toUse.getItemId()), true);
        c.getSession().write((Object)CField.enchantResult(1));
        return true;
        } else {
            MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, scroll, (short) 1, false);
        c.getPlayer().getMap().broadcastMessage(c.getPlayer(), CField.getScrollEffect(c.getPlayer().getId(), Equip.ScrollResult.FAIL, false, toScroll.getItemId(), toUse.getItemId()), true);
        c.getSession().write((Object)CField.enchantResult(0));
        return true;
        }
    }
        ...
        if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId && !chr.hasBlockedInventory() && !chr.inPVP()) {
            switch (toUse.getItemId()) {
                ...
                case 2431394: {
                    ...
                    Equip equip1 = (Equip)ii.getEquipById(1003529);
                    Equip equip2 = (Equip)ii.getEquipById(1052457);
                    Equip equip3 = (Equip)ii.getEquipById(1072660);
                    Equip equip4 = (Equip)ii.getEquipById(1082430);
                    Equip equip5 = (Equip)ii.getEquipById(1102394);
                    equip1.setStr((short)200);
                    equip1.setDex((short)200);
                    equip1.setInt((short)200);
                    equip1.setLuk((short)200);
                    equip2.setStr((short)200);
                    equip2.setDex((short)200);
                    equip2.setInt((short)200);
                    equip2.setLuk((short)200);
                    equip3.setStr((short)200);
                    equip3.setDex((short)200);
                    equip3.setInt((short)200);
                    equip3.setLuk((short)200);
                    equip4.setStr((short)200);
                    equip4.setDex((short)200);
                    equip4.setInt((short)200);
                    equip4.setLuk((short)200);
                    equip5.setStr((short)200);
                    equip5.setDex((short)200);
                    equip5.setInt((short)200);
                    equip5.setLuk((short)200);
                    MapleInventoryManipulator.addbyItem(c, equip1);
                    MapleInventoryManipulator.addbyItem(c, equip2);
                    MapleInventoryManipulator.addbyItem(c, equip3);
                    MapleInventoryManipulator.addbyItem(c, equip4);
                    MapleInventoryManipulator.addbyItem(c, equip5);
                    c.getPlayer().dropMessage(6, "You just received Beginer's Gift Box. Have fun with Smile MS!");
                    break;
                }
                case 2430121: {
 ...
 //                           final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
    ...
    public static final void UseCashItem(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (itemId) {
            ...
            case 5062002: {
                c.getPlayer().setblackslot(slea.readInt());
                c.getSession().write(CField.enchantResult(1));//3
                        c.getSession().write(CWvsContext.enableActions());
                        MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.CASH, c.getPlayer().getInventory(MapleInventoryType.CASH).findById(5062002).getPosition(), (short) 1, false);
                NPCScriptManager.getInstance().start(c, 9010000, "bloodcube");
                break;
            }
            ...
            case 5062010:
                ...
                c.getSession().write(CField.enchantResult(1));//3
                        c.getSession().write(CWvsContext.enableActions());
                        MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.CASH, c.getPlayer().getInventory(MapleInventoryType.CASH).findById(5062010).getPosition(), (short) 1, false);
                NPCScriptManager.getInstance().start(c, 9010000, "blackcube");
                break;
                case 5062011:
                    c.getPlayer().dropMessage(1, "FUCK");
                /*c.getPlayer().setblackslot(slea.readInt());
                c.getSession().write(CField.enchantResult(1));//3
                        c.getSession().write(CWvsContext.enableActions());
                        MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.CASH, c.getPlayer().getInventory(MapleInventoryType.CASH).findById(5062010).getPosition(), (short) 1, false);
            ...
            c.getSession().write(CWvsContext.enableActions());
                c.getPlayer().turnpetvac();
                c.getPlayer().dropMessage(5, "Pet VAC is now "+(c.getPlayer().getpetvac() ? "enabled." : "disabled."));
            }
...
public static final boolean UseTeleRock(LittleEndianAccessor slea, MapleClient c, int itemId) {
    ...
    public static void soiEquip(MapleClient c, Equip toSoi, byte eqSlot) {
        Equip eqq = toSoi;
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        int reqLevel = ii.getReqLevel(toSoi.getItemId()) / 10;
        LinkedList<List<StructItemOption>> pots = new LinkedList<List<StructItemOption>>(ii.getAllPotentialInfo().values());
        int new_state = Math.abs(eqq.getPotential1());
        ...
        while (eqq.getState() != new_state) {
            for (int i = 0; i < lines; ++i) {
                ...
                while (!rewarded) {
                    StructItemOption pot = pots.get(Randomizer.nextInt(pots.size())).get(reqLevel);
                    if (pot == null || pot.reqLevel / 10 > reqLevel || !GameConstants.optionTypeFits(pot.optionType, eqq.getItemId()) || !GameConstants.potentialIDFits(pot.opID, new_state, i) || !InventoryHandler.isAllowedPotentialStat(eqq, pot.opID)) continue;
                    if (i == 0) {
                        eqq.setPotential1(pot.opID);
                    } else if (i == 1) {
                        eqq.setPotential2(pot.opID);
        ...
        c.getPlayer().forceReAddItem(toSoi, eqSlot >= 0 ? MapleInventoryType.EQUIP : MapleInventoryType.EQUIPPED);
    }
    public static boolean UseBonusScroll(LittleEndianAccessor slea, MapleClient c) {
        byte new_state = 17;
        c.getPlayer().updateTick(slea.readInt());
        byte scroll = (byte)slea.readShort();
        byte dst = (byte)slea.readShort();
        ...
        if (toUse.getItemId() != 2048306) {
            ...
            if (Randomizer.nextInt(100) < rate) {
                ...
                boolean rewarded;
                while (toScroll.getBonusState() != new_state) {
                    rewarded = false;
                    while (!rewarded) {
                        pot = pots.get(Randomizer.nextInt(pots.size())).get(reqLevel);
                        if (pot == null || pot.reqLevel / 10 > reqLevel || !GameConstants.optionTypeFits(pot.optionType, toScroll.getItemId()) || !GameConstants.potentialIDFits(pot.opID, new_state, 0) || !InventoryHandler.isAllowedPotentialStat(toScroll, pot.opID)) continue;
                        toScroll.setBonusPotential1(pot.opID);
                        rewarded = true;
                    }
                }
                ...
                result = Equip.ScrollResult.SUCCESS;
            } else if (Randomizer.nextInt(100) < curserate) {
                if (ItemFlag.SHIELD_WARD.check(oldFlag)) {
                    result = Equip.ScrollResult.FAIL;
                    toScroll.setFlag((short)(oldFlag - ItemFlag.SHIELD_WARD.getValue()));
                } else {
            ...
            while (toScroll.getBonusState() != new_state) {
                for (int i = 0; i < 3; ++i) {
                    ...
                    while (!rewarded) {
                        StructItemOption pot = pots.get(Randomizer.nextInt(pots.size())).get(reqLevel);
                        if (pot == null || pot.reqLevel / 10 > reqLevel || !GameConstants.optionTypeFits(pot.optionType, toScroll.getItemId()) || !GameConstants.potentialIDFits(pot.opID, new_state, i) || !InventoryHandler.isAllowedPotentialStat(toScroll, pot.opID)) continue;
                        if (i == 0) {
                            toScroll.setBonusPotential1(pot.opID);
                        } else if (i == 1) {
                            toScroll.setBonusPotential2(pot.opID);
                        } else if (i == 2) {
                            toScroll.setBonusPotential3(pot.opID);
                        }
                        rewarded = true;
                    }
                }
        ...
        c.getPlayer().getInventory(GameConstants.getInventoryType(toUse.getItemId())).removeItem(toUse.getPosition(), (short) 1, false);
        if (result == Equip.ScrollResult.CURSE) {
            c.getSession().write((Object)CWvsContext.InventoryPacket.scrolledItem(toUse, MapleInventoryType.EQUIP, toScroll, true, false, false));
            if (dst < 0) {
        ...
        } else if (result == Equip.ScrollResult.SUCCESS) {
            c.getSession().write((Object)CWvsContext.InventoryPacket.scrolledItem(toUse, MapleInventoryType.EQUIP, toScroll, false, false, false));
        }
        c.getPlayer().getMap().broadcastMessage(c.getPlayer(), CField.getScrollEffect(c.getPlayer().getId(), result, false, toScroll.getItemId(), toUse.getItemId()), true);
        c.getSession().write((Object)CField.enchantResult(result == Equip.ScrollResult.SUCCESS ? 1 : (result == Equip.ScrollResult.CURSE ? 2 : 0)));
        if (dst < 0) {
    ...
    public static boolean UseBonusStamp(LittleEndianAccessor slea, MapleClient c) {
        int new_state = 17;
        c.getPlayer().updateTick(slea.readInt());
        byte scroll = (byte)slea.readShort();
        byte dst = (byte)slea.readShort();
        ...
        Equip.ScrollResult result = Equip.ScrollResult.FAIL;
        int reqLevel = ii.getReqLevel(toScroll.getItemId()) / 10;
        if (toScroll.getBonusPotential3() > 0 || toScroll.getBonusState() == 0) {
            c.getSession().write((Object)CField.enchantResult(0));
            c.getPlayer().dropMessage(5, "Sorry but  this equip which have 3 bonus potential lines or no potential.");
            return false;
        }
        if (toScroll.getBonusPotential2() > 0) {
            boolean rewarded = false;
            while (!rewarded) {
                StructItemOption pot = pots.get(Randomizer.nextInt(pots.size())).get(reqLevel);
                if (pot == null || pot.reqLevel / 10 > reqLevel || !GameConstants.optionTypeFits(pot.optionType, toScroll.getItemId()) || !GameConstants.potentialIDFits(pot.opID, new_state, 2) || !InventoryHandler.isAllowedPotentialStat(toScroll, pot.opID)) continue;
        ...
        } else if (toScroll.getBonusPotential1() > 0) {
            boolean rewarded = false;
            while (!rewarded) {
                StructItemOption pot = pots.get(Randomizer.nextInt(pots.size())).get(reqLevel);
                if (pot == null || pot.reqLevel / 10 > reqLevel || !GameConstants.optionTypeFits(pot.optionType, toScroll.getItemId()) || !GameConstants.potentialIDFits(pot.opID, new_state, 1) || !InventoryHandler.isAllowedPotentialStat(toScroll, pot.opID)) continue;
