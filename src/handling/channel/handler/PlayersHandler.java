...
public class PlayersHandler {
   ...
   //     }
    ...
    public static void magicWheel(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (mode == 2) {
            slea.readInt();
            short toUseSlot = slea.readShort();
            slea.readShort();
            int tokenId = slea.readInt();
            rank = tokenId % 10;
            if (c.getPlayer().getInventory(MapleInventoryType.ETC).getItem(toUseSlot).getItemId() != tokenId) {
                c.getSession().write((Object)CWvsContext.enableActions());
                return;
            }
            for (byte inv = 1; inv <= 5; inv = (byte)(inv + 1)) {
                if (c.getPlayer().getInventory(MapleInventoryType.getByType(inv)).getNumFreeSlot() >= 2) continue;
                c.getSession().write((Object)CWvsContext.magicWheel(7, null, null, 0));
                c.getSession().write((Object)CWvsContext.enableActions());
                return;
            }
            LinkedList<Integer> percents = new LinkedList<Integer>();
            LinkedList<Integer> items = new LinkedList<Integer>();
            GameConstants.loadWheelRewards(items, percents, tokenId);
            LinkedList<Integer> reward = new LinkedList<Integer>();
            int rewards = 0;
            for (int i = 0; i < 9; ++i) {
                reward.add(rewards += percents.get(i).intValue());
            }
            int rand = Randomizer.nextInt(rewards) + 1;
            int end = 0;
            while (rand > (Integer)reward.get(end)) {
                ++end;
            }
            ...
            if (!MapleInventoryManipulator.removeFromSlot_Lock(c, GameConstants.getInventoryType(tokenId), toUseSlot,(short) 1, false, false)) {
                c.getSession().write((Object)CWvsContext.magicWheel(9, null, null, 0));
                c.getSession().write((Object)CWvsContext.enableActions());
                return;
            }
            c.getSession().write((Object)CWvsContext.magicWheel(3, items, data, end));
        } else if (mode == 4) {
            String data = slea.readMapleAsciiString();
            int item = c.getPlayer().getWheelItem();
            int wheelitem = c.getPlayer().getWheelItem();
            MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
            switch (wheelitem) {
                case 5062001: {
                    MapleInventoryManipulator.addById(c, 5062001, (short)3, null);
                    c.getPlayer().dropMessage(6, "You just got 3 " + ii.getName(5062001) + " from Magic Wheel");
                    break;
                }
                case 4031879: {
                    int ba = rank == 1 ? 3 : 5;
                    MapleInventoryManipulator.addById(c, 4031879, (short)ba, null);
                    c.getPlayer().dropMessage(6, "You just got " + ba + " " + ii.getName(4031879) + " from Magic Wheel");
                    break;
                }
                case 4033570: {
                    int aa = rank == 1 ? Randomizer.nextInt(4000000) + 1000001 : Randomizer.nextInt(9000000) + 1000001;
                    c.getPlayer().gainMeso(aa, true);
                    c.getPlayer().dropMessage(6, "You just got " + aa + " Mesos from Magic Wheel");
                    break;
                }
                case 5750000: {
                    MapleInventoryManipulator.addById(c, 5750000, (short)3, null);
                    c.getPlayer().dropMessage(6, "You just got 3 " + ii.getName(5750000) + " from Magic Wheel");
                    break;
                }
                case 5060004: {
                    MapleInventoryManipulator.addById(c, 5060004, (short)3, null);
                    c.getPlayer().dropMessage(6, "You just got 3 " + ii.getName(5060004) + " from Magic Wheel");
                    break;
                }
                case 5062090: {
                    MapleInventoryManipulator.addById(c, 5062090, (short)5, null);
                    c.getPlayer().dropMessage(6, "You just got 5 " + ii.getName(5062090) + " from Magic Wheel");
                    break;
                }
                case 5062002: {
                    int b = rank == 1 ? 2 : 3;
                    MapleInventoryManipulator.addById(c, 5062002, (short)b, null);
                    c.getPlayer().dropMessage(6, "You just got " + b + " " + ii.getName(5062002) + " from Magic Wheel");
                    break;
                }
                case 4035000: {
                    c.getPlayer().setDPoints(c.getPlayer().getDPoints() + 10);
                    c.getPlayer().dropMessage(6, "You just got 10 Points from Magic Wheel");
                    break;
                }
                case 4035001: {
                    c.getPlayer().setDPoints(c.getPlayer().getDPoints() + 20);
                    c.getPlayer().dropMessage(6, "You just got 20 Points from Magic Wheel");
                    break;
                }
                case 4035002: {
                    c.getPlayer().setDPoints(c.getPlayer().getDPoints() + 200);
                    c.getPlayer().dropMessage(6, "You just got 200 Points from Magic Wheel");
                    break;
                }
                case 4035003: {
                    c.getPlayer().setDPoints(c.getPlayer().getDPoints() + 500);
                    c.getPlayer().dropMessage(6, "You just got 500 Points from Magic Wheel");
                    break;
                }
                case 4035004: {
                    c.getPlayer().setDPoints(c.getPlayer().getDPoints() + 1000);
                    c.getPlayer().dropMessage(6, "You just got 1000 Points from Magic Wheel");
                    break;
                }
                case 4035005: {
                    c.getPlayer().modifyCSPoints(2, 2000);
                    c.getPlayer().dropMessage(6, "You just got 2000 NX from Magic Wheel");
                    break;
                }
                case 1082149: {
                    int ab = rank == 0 ? 200 : 500;
                    Equip equip = (Equip)ii.getEquipById(1082149);
                    equip.setStr((short)ab);
                    equip.setModSlot((byte)50);
                    equip.setReqLevel((byte) 100);
                    equip.setDex((short)ab);
                    equip.setInt((short)ab);
                    equip.setLuk((short)ab);
                    MapleInventoryManipulator.addbyItem(c, equip);
                    c.getPlayer().dropMessage(6, "You just got a(n) " + ii.getName(1082149) + " + " + ab + " ALL STAT from Magic Wheel");
                    break;
                }
                case 1082148: {
                    Equip equip = (Equip)ii.getEquipById(1082148);
                    equip.setStr((byte) 350);
                    equip.setDex((byte) 350);
                    equip.setInt((byte) 350);
                    equip.setLuk((short) 350);
                    equip.setModSlot((byte) 50);
                    equip.setReqLevel((byte) 100);
                    MapleInventoryManipulator.addbyItem(c, equip);
                    c.getPlayer().dropMessage(6, "You just got a(n) " + ii.getName(1082148) + " + 350 ALL STAT from Magic Wheel");
                    break;
                }
                default: {
                    if (GameConstants.isPet(wheelitem)) {
                        MaplePet pet = MaplePet.createPet(wheelitem, wheelitem);
                        MapleInventoryManipulator.addById(c, wheelitem, (short) 1, null, pet, null);
                    } else {
                        MapleInventoryManipulator.addById(c, wheelitem, (short) 1, null);
                    }
                    c.getPlayer().dropMessage(6, "You just got a(n)" + ii.getName(wheelitem) + " from Magic Wheel");
                }
            }
