...
//            return true;
        ...
        public int getEqpStat(byte slot, int type) {
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        Equip b = (Equip)this.c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem(slot);
        int a = 0;
        switch (type) {
            case 0: {
                a = b.getStr();
    ...
    public void renewptt(byte slot, int type) {
        Equip eq = (Equip)this.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem(slot);
        if(type  == 10){
         
             int state = eq.getBonusState();
             MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        ...
        if(Randomizer.nextInt(100)<4 && state <20)
                 state++;//4% chance to up tier;
             do {
                for (int i = 0; i < 3; ++i) {
                    boolean rewarded = false;
                    while (!rewarded) {
                        StructItemOption pot = pots.get(Randomizer.nextInt(pots.size())).get(reqLevel);
                        if (pot == null || pot.reqLevel / 10 > reqLevel || !GameConstants.optionTypeFits(pot.optionType, eq.getItemId()) || !GameConstants.potentialIDFits(pot.opID, state, i) || !InventoryHandler.isAllowedPotentialStat(eq, pot.opID)) continue;
                        if (i == 0 && eq.getBonusPotential1() > 0) {
                            eq.setBonusPotential1(pot.opID);
                            } else if (i == 1 && eq.getBonusPotential2() > 0) {
                            eq.setBonusPotential2(pot.opID);
                            } else if (i == 2 && eq.getBonusPotential3() > 0) {
                            eq.setBonusPotential3(pot.opID);
                            }
                        rewarded = true;
                    }
                }
            }  while (eq.getBonusState() != state);         //c.getPlayer().forceReAddItem(eq, MapleInventoryType.EQUIP);
             getPlayer().forceReAddItem(eq, MapleInventoryType.EQUIP);
        } else{
        eq.renewRed(false, 0);
        this.getPlayer().forceReAddItem_NoUpdate(eq, MapleInventoryType.EQUIP);
        InventoryHandler.soiEquip(this.c, eq, (byte)eq.getPosition());
        }
    }

    public void modStat(byte slot, int type, int amount) {
        Equip a = (Equip)this.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem(slot);
        Equip b = (Equip)a.copy();
        short cStr = b.getStr();
        short cDex = b.getDex();
        short cInt = b.getInt();
        short cLuk = b.getLuk();
        switch (type) {
            case 0: {
                b.setStr((short)(amount + cStr));
                break;
            }
            case 1: {
                b.setDex((short)(amount + cDex));
                break;
            }
            case 2: {
                b.setInt((short)(amount + cInt));
                break;
            }
            case 3: {
                b.setLuk((short)(amount + cLuk));
            }
        }
        b.setModSlot((byte)(b.getModSlot() - 1));
        this.getPlayer().forceReAddItem(b, MapleInventoryType.EQUIP);
    }

    public static String resolvePotentialID(int itemID, int potID) {
        int potLevel;
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        int eqLevel = ii.getReqLevel(itemID);
        List<StructItemOption> potInfo = MapleItemInformationProvider.getInstance().getPotentialInfo(potID);
        if (eqLevel == 0) {
            potLevel = 1;
        } else {
    ...
    public String getLG() throws SQLException {
        StringBuilder sb = new StringBuilder();
        this.getChannelServer().getLG().loadPlayers();
        List<Integer> list = this.getChannelServer().getLG().getPlayers();
        if (list.isEmpty()) {
            sb.append("No one join yet. You might want to join? Try it !");
        } else {
            MapleClient c = new MapleClient(null, null, (IoSession)new MockIOSession());
            sb.append("List of player joined Lucky Guy today:\r\n");
            for (int i = 0; i < list.size(); ++i) {
                MapleCharacter a = MapleCharacter.loadCharFromDB(list.get(i), c, true);
                sb.append("" + (i + 1) + " - " + a.getName() + ".\r\n");
            }
        }
        return sb.toString();
    }
    public void openScript(int idnpc, String script){
        c.removeClickedNPC();
        NPCScriptManager.getInstance().start(c, idnpc, script);
    }
  }