    ...
    extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (c.getPlayer().getLevel() < 250) {
                c.getPlayer().dropMessage(5, "You cannot Rebirth until you reach level 250. Please try again later.");
                return 0;
            }
            c.getPlayer().doReborn();
            c.getPlayer().dropMessage(5, "Congratulation! You has been rebith successfully. Now you has rebirth " + c.getPlayer().getReborns() + " times");
            World.Broadcast.broadcastMessage(CWvsContext.broadcastMsg(6, "Congratulation! " + c.getPlayer().getName() + " just rebirth successfully "+ c.getPlayer().getReborns() + " times."));
            return 1;
        }
    }
}
