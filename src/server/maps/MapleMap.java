    ...
    public final void removeMapObject(final MapleMapObject obj) {
        ...
        if (this.getFieldType().equals("63")) {
            for (MapleMapObject object : this.getMapObjectsInRange(chr.getPosition(), Double.POSITIVE_INFINITY, Arrays.asList(MapleMapObjectType.MIST))) {
                ...
                //  System.out.println("미스트!");
                if (clock.isClock()) {
                    //System.out.println("시계일시");
                    ...
                    if (rect.intersects(monster.getRectangle())) {
                        // System.out.println("포함할시");
                        clock.setUsed(true);
                      //  chr.send(UIPa("반반이 시간을 움직임"));
                        return;
                    }
                    //}
                }
...
//            FileoutputUtil.log(FileoutputUtil.Zakum_Log, MapDebug_Log());
   ...
   //   MapleCharacter chr = (MapleCharacter);
        ...
        if(getChannel() == 4 || getChannel() == 5) {
            OverrideMonsterStats overrideStats = new OverrideMonsterStats(monster.getMobMaxHp() * 2, monster.getMobMaxMp(), monster.getMobExp() * 2);
            monster.setOverrideStats(overrideStats);
            monster.getChangedStats().MDRate*=2;
            monster.getChangedStats().PDRate*=2;
            monster.getChangedStats().level*=2;
        }
        spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
            @Override
            public final void sendPackets(MapleClient c) {
